...
export declare const isErrorLike: (maybe: unknown) => maybe is import("./object/type/ErrorLike").ErrorLike, chain: <T>(initial: T, mappers: ((v: T) => T)[]) => T, entries: <O extends object, K extends keyof O, V = O[K]>(obj: O) => [K, V][], filter: <O extends object, K extends keyof O, V extends O[K]>(obj: O, predicate: (key: K, value: V) => boolean) => [K, V][], first: <O, T extends O>(obj: Readonly<O | Iterator<O, T, undefined> | O[]>, typeGuard?: import("./object/type/TypeGuard").TypeGuard<T>) => T | undefined, last: <T>(obj: readonly T[]) => T | undefined, forEach: <O extends object, K extends keyof O>(obj: O, consumer: (key: K, value: O[K]) => void, errorHandler?: (error: unknown, key: K, value: O[K]) => void) => void, iff: <T, R>(value: T, mapper: (v: NonNullable<T>) => R) => R | undefined, orElse: <T>(o: T, e: T) => T, removeUndefinedValues: <T extends object>(obj: object) => Partial<T>, iffTyped: <T, R, G>(typeGuard: (maybe: unknown) => maybe is G, value: T, mapper: (v: T & G) => R) => R | undefined, isDefined: <T>(obj: T | null | undefined) => obj is NonNullable<T>, isUndefined: <T>(obj: T | null | undefined) => obj is undefined, omit: <K extends string | number, T extends { [k in K]: unknown; }>(object: T, key: K) => Omit<T, K>, omitUnsafe: <K extends string | number, T extends { [k in K]: unknown; }>(object: T, key: K) => Omit<T, K>, toMany: <T>(obj: T | Iterable<T> | T[]) => T[], fromEntries: <K extends string | number | symbol, V>(entries: readonly (readonly [K, V])[]) => Record<K, V>, safe: <R>(producer: () => R, options?: Partial<{
    ...
    def: R;
    onError: import("./object/type/Producer").ValueProducer<string>;
}>) => Promise<R | undefined>, mapValue: <T, R>(value: T, mapper: (value: T) => R) => R, headOf: <O, T extends O>(obj: Readonly<O | Iterator<O, T, undefined> | O[]>, typeGuard?: import("./object/type/TypeGuard").TypeGuard<T>) => T | undefined, tailOf: <O>(obj: O[]) => O[] | undefined, restOf: <O>(obj: O[]) => O[] | undefined, times: typeof import("./object/times").times, isIterator: <T>(maybe: unknown) => maybe is Iterator<T, any, undefined>, toMultiMap: <K extends string | number, T>(maps: Record<K, T>[], options?: Partial<{
    compact: boolean;
