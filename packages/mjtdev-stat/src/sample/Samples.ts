...
const stackToParentName = (stack: string, startFrom: number = 3) => {
...
const create = (sample: Partial<Sample> = {}): Sample => {
  const {
    name = `${stackToParentName(new Error().stack)}`,
    values = [],
    logEnabled = true,
    lastLogged = 0,
    clearAfter = "tick",
...
const add = (value: unknown, sample: Partial<Sample> = {}) => {
...
const get = (sample: Partial<Sample>): Sample => {
  const { name } = sample;
