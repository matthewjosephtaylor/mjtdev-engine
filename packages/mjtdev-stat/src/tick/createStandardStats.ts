import { PerformanceMemory } from "../type/PerformanceMemory";
import { toMB } from "./toMB";
import { StatUpdater } from "./type/StatUpdater";


export const createStandardStats = (): StatUpdater[] => {
  let deltas: number[] = [];
  let costs: number[] = [];
  return [
    {
    ...
    },
    {
      ...
      tickToStat: (tick) => {
        if (tick.frameCount % 30 === 0) {
          const memory: PerformanceMemory = performance["memory"];
          return `${toMB(memory.jsHeapSizeLimit)}/${toMB(
